...
class AnnealAfterPrint(Script):
    ...
    def getSettingDataString(self):
        ...
        return """{
            ...
            "settings":
            {
                ...
                "movePrintheadToOrigin":
                {
                    "label": "Move printhead to origin",
                    "description": "Move printhead X/Y to origin (5,5) of printer after the print ends, before annealing. This will create enough clearance in the printer gantry for placement of an insulating cover over the printed object. ",
                    "type": "bool",
                    "default_value": true
                },
                "endCoolingTemp":
    ...
    def generateAnnealCode(self, annealBedTemp, annealChamberTemp, annealMinutes, initialBeep, movePrintheadToOrigin, endCoolingTemp, coolingRate):
        anneal_code = ';Generated Annealing GCODE by Anson Liu'

        if initialBeep:
            anneal_code += '\nM300 ;play beep for container placement reminder'

        if movePrintheadToOrigin:
            anneal_code += '\nG90 '
            anneal_code += '\nG0 X5 Y5 F3000 '
    ...
    def execute(self, data):
        ...
        move_printhead_to_origin = self.getSettingValueByKey("movePrintheadToOrigin")
        ...
        anneal_code = self.generateAnnealCode(anneal_bed_temp, anneal_chamber_temp, anneal_minutes, initial_beep, move_printhead_to_origin, final_cooling_temp, cooling_rate)
