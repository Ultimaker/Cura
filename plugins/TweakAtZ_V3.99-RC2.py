...
#V3.99-RC1:   Progress bar, tweaking over multiple layers, M605&M606 implemented, reset after one layer option, extruder three code removed, tweaking print speed
#V3.99-RC2:   save call of Publisher class, uses previous value from other plugins also on UltiGCode
...
old = {'speed': -1, 'flowrate': -1, 'flowrateOne': -1, 'flowrateTwo': -1, 'platformTemp': -1, 'extruderOne': -1, 'extruderTwo': -1, 'fanSpeed': -1, 'state': -1}
try:
...
IsUM2 = False #Used for reset of values (ok for Marlin/Sprinter), has to be set to 1 for UltiGCode (work-around for missing default values)
lastpercentage = 0
i = 0
l = len(lines)
...
with open(filename, "w") as file:
	for line in lines:
		...
		if not ('M84' in line or 'M25' in line or ('G1' in line and TweakPrintSpeed and state==3) or ';TweakAtZ instances:' in line):
			file.write(line)
