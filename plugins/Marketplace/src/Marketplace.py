...
##  The Marketplace class is responsible of communicating with the server through the API
class Marketplace(QObject, Extension):
    ...
    def browsePackages(self) -> None:
        ...
        if not self._dialog:
            self._dialog = self._createDialog("Marketplace.qml")
    ...
    def _updateInstalledModels(self) -> None:
        ...
        if "plugin" in all_packages:
            # For old plugins, we only want to include the old custom plugin that were installed via the old marketplace.
    ...
    def checkPackageUsageAndUninstall(self, package_id: str) -> None:
        ...
        if package_used_materials or package_used_qualities:
            ...
            if self._confirm_reset_dialog is None:
                self._confirm_reset_dialog = self._createDialog("MarketplaceConfirmUninstallResetDialog.qml")
            self.uninstallVariablesChanged.emit()
            if self._confirm_reset_dialog is None:
                Logger.log("e", "MarketplaceConfirmUninstallResetDialog should have been initialized, but it is not. Not showing dialog and not uninstalling package.")
            else:
