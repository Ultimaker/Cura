...
Item
{
    ...
    height: visible ? Math.floor(UM.Theme.getSize("marketplace_footer").height) : 0
    Label
    {
        text: catalog.i18nc("@info", "You will need to restart Cura before changes in packages have effect.")
        color: UM.Theme.getColor("text")
        height: Math.floor(UM.Theme.getSize("marketplace_footer_button").height)
    ...
    Button
    {
        ...
        onClicked: marketplace.restart()
        style: ButtonStyle
        {
            background: Rectangle
            {
                implicitWidth: UM.Theme.getSize("marketplace_footer_button").width
                implicitHeight: Math.floor(UM.Theme.getSize("marketplace_footer_button").height)
