...
class CuraConan(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("curaengine/5.5.0-beta.1")
        self.requires("pysavitar/5.3.0")
        self.requires("pynest2d/5.3.0")
        self.requires("curaengine_plugin_gradual_flow/0.1.0-beta.1")
        self.requires("uranium/5.5.0-beta.1")
        self.requires("cura_binary_data/5.5.0-beta.1")
        self.requires("cpython/3.10.4")
        if self.options.internal:
            self.requires("cura_private_data/(latest)@ultimaker/testing")
            self.requires("fdm_materials/(latest)@internal/testing")
        else:
            self.requires("fdm_materials/5.5.0-beta.1")
